...
def superadmin_remove(request, user_id):
    ...
    if superadmin.user.username == current_user.username:
        return settings(request, alert_head="Deletion failed",
                        alert_text="Don't torture yourself.")
    ...
    return settings(request, alert_type="success",
                    alert_head="Deletion succeeded",
                    alert_text="Super Admin Deleted.")
...
def superadmin_add(request):
    ...
    try:
        ...
        print(e)
        return settings(request, alert_head='Grant Failed',
    ...
    try:
        ...
        return settings(request, alert_head="Grant Failed",
                        alert_text="Unable to make user " + str(
                            user.username) + " a super admin. ")

    return settings(request, alert_type="success",
                    alert_head="Grant succeeded",
                    alert_text="User " + str(user.username) + " is a super "
                                                              "admin now.")
