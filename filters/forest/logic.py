...
def parse_binary(tree):
    ...
    if (tree["type"] == 'UnaryExpression'):
        ...
        if tree["operator"] in OP_NOT:
            ...
            raise Exception(
                'Expected a binary expression, but found unknown '
                'UnaryExpression type in input. ')

    # check for binary logical connectives
    if tree["type"] == 'BinaryExpression' \
            or tree["type"] == 'LogicalExpression':
    ...
    if tree["type"] == 'BinaryExpression' \
        ...
        raise Exception(
            'Expected a binary expression, but found unknown '
            'BinaryExpression / LogicalExpression in input. ')
    ...
    if is_primitive:
        ...
        raise Exception(
            "Expected a binary expression, but found unknown expression type "
            "in input. ")
...
def parse_primitive(tree):
    ...
    A primitive expression is either:
      ...
      3. A non-empty compound expression consisting of primitives
      as above ( ==> join as strings )
    ...
    raise Exception(
        "Expected a primitive expression, but found unknown expression type "
        "in input. ")
